...
describe("selectRaw / css", () => {
  it("reads text content of the first match by default", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: "$19.99", rawList: null, missing: false }))
  })
  ...
  it("reads the last match", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: "$1.00", rawList: null, missing: false }))
  })
  ...
  it("reads a specific nth match", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: "$9.99", rawList: null, missing: false }))
  })
  ...
  it("reads all matches as a list", async () => {
    ...
    expect(exit).toEqual(
      Exit.succeed({ raw: null, rawList: ["$19.99", "$9.99", "$1.00"], missing: false }),
    )
  })
  ...
  it("reads an attribute instead of text when configured", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: "/product/1", rawList: null, missing: false }))
  })
  ...
  it("reports missing when nothing matches", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: null, rawList: null, missing: true }))
  })
  ...
  it("respects the content_selector scope", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: null, rawList: ["$19.99", "$9.99"], missing: false }))
  })
...
describe("selectRaw / xpath", () => {
  it("reads matching text via a lite xpath expression", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: "new", rawList: null, missing: false }))
  })
})
...
describe("selectRaw / jsonpath", () => {
  ...
  it("reads a value from a json response body", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: "42.5", rawList: null, missing: false }))
  })
  ...
  it("reads a list from a json response body", async () => {
    ...
    expect(exit).toEqual(Exit.succeed({ raw: null, rawList: ["a", "b"], missing: false }))
  })
})
