...
```ts
// openapi-ts.config.ts (repo root) — see 04-FRONTEND §5 for the full file
export default defineConfig({
  input: "apps/api/openapi.json", // committed, regenerated by `pnpm gen:openapi`
  output: { path: "apps/web/src/api/generated", postProcess: [/* prettier, seal */] },
  plugins: [
...
**The document declares `servers: [{ url: ROUTE.apiBase }]`.** Without it the
generator infers a base URL from the input _path_ and bakes nonsense
(`baseUrl: 'apps'`) into `client.gen.ts`. The runtime config overrides it anyway,
but the fallback should still be right.
