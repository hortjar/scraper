...
- [ ] **Config** — every env var from [11-ENVIRONMENT](./11-ENVIRONMENT.md) as an
      Effect `Config`; secrets `Config.redacted`; fail-fast listing _all_ problems;
...
- [x] `apps/web`: Vite, React 19, Tailwind 4 with the design tokens from
      ...
      Query providers, landing route and console shell. **Fonts are not in the repo** —
      `public/fonts/` holds a `.gitkeep`; see `apps/web/src/assets/fonts/README.md`.
- [x] **API pipeline**: `pnpm gen:openapi` → `apps/api/openapi.json` → `pnpm gen:api`
      → `apps/web/src/api/generated`. Both committed; CI regenerates and runs
      `git diff --exit-code`.
