...
no cross-imports, acyclic dependencies) comes from *module boundaries and lint
rules*, not from `package.json` files. Fourteen packages would have meant fourteen
...
export const effectPlugin = new Elysia({ name: PLUGIN.effect })
  .decorate('runFx', <A, E extends AppError>(fx: Effect.Effect<A, E, AppServices>) =>
...
- **Redis down** → no jobs execute; API stays up read-only for job creation
  ...
  *what should be scheduled*; Redis holds only *execution state*).
