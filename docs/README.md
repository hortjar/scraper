...
| Workspace shape | **4 packages, 3 apps** | `core` · `db` · `server` · `tooling`. Features are *modules* inside `server`, not packages — same isolation, a tenth of the ceremony. |
...
| API docs | **Swagger UI** at `/docs` | Plus Scalar at `/docs/scalar`, OpenAPI 3.1 at `/docs/json`. |
...
- **Atomic components.** One job each, ≤150 lines, fetch *or* render — never both.
