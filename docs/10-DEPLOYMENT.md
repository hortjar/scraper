...
- **Nothing builds these automatically.** Compose builds them from source with the
  build config in `docker-compose.yml`, which is the default path; a registry is
  optional. See §8.
...
├── docker-compose.registry.yml     overlay: pull published images instead of building
